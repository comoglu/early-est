...
class ProcessEvents(object):
    ...
    def createMapLink(self, event_id):
        ...
        # map_link = "http://maps.google.com/maps?q={0:g}+{1:g}({2:s})&f=d&t=h&hl=e&ie=UTF8&ll={3:g},{4:g}&z=5".format(lat, lon, hypo_description, lat, lon)
        # clean link
    ...
    def writeHtmlEventInfo(self, instance_path_name, event_id, event_root_node, mech_root_nodes_list, event_info_last_update_time):
        ...
        titleRoot1+= region + self.space(4) + origin_time \
            + self.space(4)
        titleRoot2 = self.space(4) + "mb:" + mag_mb_display + self.space(4) + "Mwp:" + mag_Mwp_display + self.space(4) + "Mwpd:" \
            + mag_Mwpd_display + self.space(4) + "Td*T50Ex:" + mag_wl_display + " "
 ...
 = evaluateEvent.findClosestPlateBoundary(hypo_lat, hypo_lon, exclude_boundary_codes)
            ...
            print "error: generating calculated values in EvlauateEvent:", str(e)
        ...
        tableData.append(Tag.FONT(self.space(1) + "[+/-" + '{0:.0f}'.format(float(parseQML.getValueInFirstChildWithName(origin_node, "depth", "uncertainty"))) + "km]", tag_class="small"))
        tableRow.append(tableData)
        table.append(tableRow)
        ...
        ellipsoid = parseQML.getValueOfFirstChildWithName(confidence_ellipsoid_node, "confidenceLevel") + "%ConfEllipsoid:" + \
            self.space(3) + "semiMajorAxisLength: " + '{0:.0f}'.format(float(parseQML.getValueOfFirstChildWithName(confidence_ellipsoid_node, "semiMajorAxisLength"))) + "km" + \
            ...
            self.space(3) + "semiIntermediateAxisLength: " + '{0:.0f}'.format(float(parseQML.getValueOfFirstChildWithName(confidence_ellipsoid_node, "semiIntermediateAxisLength"))) + "km" + \
            ...
            self.space(3) + "semiMinorAxisLength: " + '{0:.0f}'.format(float(parseQML.getValueOfFirstChildWithName(confidence_ellipsoid_node, "semiMinorAxisLength"))) + "km"
        ...
        if flag_config__event_info__tsunami_write:
            ...
            # tsunamiLearn and ruptureLearn assesment
            if flag_config__event_info__tsunami_rupture_predictions_write:
                ...
                if len(mech_root_nodes_list) > 0:
                    ...
                    for process_type, pkl_file_root in [ \
                        ...
                        #print 'DEBUG: process_type:', process_type, 'pkl_file_root:', pkl_file_root
                        if (not pkl_file_root == None):
                            ...
                            print 'DEBUG: process_type:', process_type, 'prediction:', prediction, 'ipercent_prob_underwater:', ipercent_prob_underwater, 'Ndip:', Ndip
                            text, fmt, units = tsunami_rupture_predictions_text[process_type]
                            tableData.append(text + ': ' + fmt.format(float(prediction)) + units)
                            tableData.append(Tag.BR())
                else:
                    tableData.append('ERROR: No mechanism available.')
                    tableData.append(Tag.BR())
    ...
    def mech2qml(self, f_mech_in, resource_id_root, event_parameters_public_id, event_public_id, event_id, type):
        ...
        elif type == MECH_FMAMP_POL or type == MECH_FMAMP_AMP_AREF or type == MECH_FMAMP_AMP_MWP:
        ...
        focalMechanism.setAttribute("publicID", resource_id_root + "/origin/" + event_id + "/focal_mechanism/" + type)
        event.appendChild(focalMechanism)

        # triggeringOriginID
        triggeringOriginID = doc.createElement("triggeringOriginID")
        textNode = doc.createTextNode(resource_id_root + "/origin/" + event_id)
    ...
    def processEvents(self, save_dir, root_out_path, instance_path_name, instance_name, plot_map_mechanism_type):
        ...
        # parse events in monitor.xml, run plot_event_info.gmt, write mechanism qml and event html
        # parse monitor.xml
        t1 = time.time()
                    ...
                    if (node1.nodeName == "eventParameters"):
                        ...
                        for node2 in node1.childNodes:
                            if (node2.nodeName == "event"):
                                ...
                                # run plot_event_info.gmt
                                plot_mechanisms = "NO"
                                if flag_config__event_info__first_motion_mech_run:
                                    plot_mechanisms = "YES"
                                subprocess_cmd = cwd_path + "/plot_event_info.gmt " + save_dir + " " + root_out_path + " " + instance_path_name + " " + instance_name \
                                + " " + event_id + " " + region.replace(" ", "_") + " " + origin_time + " " + plot_mechanisms + " " + plot_map_mechanism_type
                                print subprocess_cmd
                                args = shlex.split(subprocess_cmd)
                                t2 = time.time()
                                retcode = subprocess.call(args)
                                print "DEBUG: TIME: plot_event_info.gmt: total time = %0.3fs" % ((time.time() - t2))
                                # write mechanism qml
                                mech_root_nodes_list = []
                                # order of event types here determines order displaysed in HTML event page, see also mech type for map plotting in plot_warning_report.gmt, plot_event_info.gmt
                                ...
                                try:
                                    ...
                                    fmamp_mech_doc = self.mech2qml(f_mech_in, resource_id_root, event_parameters_public_id, event_public_id, event_id, MECH_FMAMP_POL)
                                    f_mech_in.close()
                                    ...
                                    f_mech_out.write(fmamp_mech_doc.toprettyxml("   ", encoding="utf-8"))
                                    f_mech_out.close()
                                ...
                                try:
                                    ...
                                    f_mech_in = open(fname, "r")
                                    hash_mech_doc = self.mech2qml(f_mech_in, resource_id_root, event_parameters_public_id, event_public_id, event_id, MECH_HASH)
                                    f_mech_in.close()
                                    mech_root_nodes_list.append(("hash", hash_mech_doc))
                                    # write qml
                                    fname = instance_path_name + "/events/hypo." + event_id + ".mech.hash.xml"
                                    f_mech_out = open(fname, "w")
                                    f_mech_out.write(hash_mech_doc.toprettyxml("   ", encoding="utf-8"))
                                    # hash_mech_doc.writexml(f_mech_out, "   ", encoding="utf-8")
                                    f_mech_out.close()
                                ...
                                try:
                                    ...
                                    f_mech_in = open(fname, "r")
                                    fmamp_mech_doc = self.mech2qml(f_mech_in, resource_id_root, event_parameters_public_id, event_public_id, event_id, MECH_FMAMP_AMP_AREF)
                                    f_mech_in.close()
                                ...
                                try:
                                    ...
                                    f_mech_in = open(fname, "r")
                                    fmamp_mech_doc = self.mech2qml(f_mech_in, resource_id_root, event_parameters_public_id, event_public_id, event_id, MECH_FMAMP_AMP_MWP)
                                    f_mech_in.close()
...
if __name__ == "__main__":
   ...
   # focal mechanism
    ...
    flag_config__event_info__first_motion_mech_run = flag_config__event_info__first_motion_mech_run_hash \
        or flag_config__event_info__first_motion_mech_run_fmamp_pol or flag_config__event_info__first_motion_mech_run_fmamp_amp
    try:
