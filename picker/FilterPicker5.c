...
void Pick(
        ...
        // 20220202 AJL - bug fix, added ppick_list_size parameter to hold current size of allocated array
        int* ppick_list_size, // the allocated size of array *ppick_list
    ...
    for (n = 0; n < num_samples; n++) {
        ...
        for (k = mem->numRecursive - 1; k >= 0; k--) {
            ...
            if (mem->mean_stdDev_xRec[k] <= DOUBLE_MIN_VALUE) {
                if (mem->enableTriggering && error1_not_printed) {
                    ...
                    if (0) { // 20131202 AJL - Debug
                        sprintf(message_str, "DEBUG: %s, k=%d, currentSample=%f, currentDiffSample=%f, mem->xRec[k]=%f, mean_stdDev_xRec[k]=%f\n",
                                channel_id, k, currentSample, currentDiffSample, mem->xRec[k], mem->mean_stdDev_xRec[k]);
                        info(message_str);
                    }
                }
            } else {
        ...
        if (resultType == TRIGGER) { // show triggers
            ...
            if (acceptedPick) {
                ...
                addPickToPickList(pickData, ppick_list, ppick_list_size, pnum_picks);
