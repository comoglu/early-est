...
int main(int argc, char *argv[]) {
    ...
    int pick_list_definative_size = 0;
    ...
    while ((read_samples = fread(sample, sizeof (float), length, fp)) != 0) { //loop over trace
        ...
        for (n = 0; n < num_picks; n++) {
            ...
            addPickToPickList(pick, &pick_list_definative, &pick_list_definative_size, &num_picks_definative);
        }
    ...
    strncpy(kstnm, sachdr.KSTNM, sizeof (sachdr.KSTNM));
    char* kinst;
    kinst = calloc(1, 16 * sizeof (char));
    strncpy(kinst, sachdr.KINST, sizeof (sachdr.KINST));
    if (strstr(kinst, "(count") != NULL)
        strcpy(kinst, "(counts)");
    char* kcmpnm;
    kcmpnm = calloc(1, 16 * sizeof (char));
    strncpy(kcmpnm, sachdr.KCMPNM, sizeof (sachdr.KCMPNM));
    char phase[16];
    // create NLL picks
    char* pick_str;
