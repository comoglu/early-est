...
void addPickToPickList(PickData* pickData, PickData*** ppick_list, int *sizeofPickList, int* pnum_picks) {

    // 20220202 AJL - bug fix, added sizeofPickList parameter to hold current size of allocated array
    ...
    if (*ppick_list == NULL) { // list not yet created
        *ppick_list = calloc(SIZE_INCREMENT, sizeof (PickData*));
        *sizeofPickList = SIZE_INCREMENT;
        // 20130930 AJL - bug fix
        //}else if ((*pnum_picks % SIZE_INCREMENT) == 0) {	// list will be too small
    } else if (*pnum_picks + 1 > *sizeofPickList) { // list will be too small
        newPickList = calloc(*pnum_picks + SIZE_INCREMENT, sizeof (PickData*));
        *sizeofPickList = *pnum_picks + SIZE_INCREMENT;
